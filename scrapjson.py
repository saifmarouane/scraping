...
json_file_path = 'breadcrumbs_data.json'
...
if 'Design Library' in driver.page_source:
    ...
    def extract_links(driver, url, depth=0, parent_url=None):
        ...
        try:
            ...
            print(select.select_by_value('659'))
            if select:
                ...
                if download_link:
                    ...
                    # Extraire tous les noms des éléments du breadcrumb
                    breadcrumbs = driver.find_elements(By.CSS_SELECTOR, "div.breadcrumb.mb-4 a")
                    # Collecter les titres dans une liste
                    

                    if breadcrumbs:
                        for crumb in breadcrumbs:
                            a=crumb.get_attribute('title')                            
                            titles += a + ">"
                        if titles:  # Remove the trailing ">" if titles is not empty
                            
                            titles = titles[:-1]
                        print(titles)
                        breadcrumbs_data.append({"title": titles, "href": download.get_attribute('href')})

                        # Écrire les données dans un fichier JSON
                        with open(json_file_path, 'w') as json_file:
                            json.dump(breadcrumbs_data, json_file, indent=4)
